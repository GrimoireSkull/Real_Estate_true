...
function Mortgage() {
  ...
  const handleCalculate = () => {
    ...
    setMonthlyRepayment(calculatedMonthlyRepayment.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')); // Menambahkan koma setiap 3 digit
    const calculatedTotalPayable = calculatedMonthlyRepayment * numberOfPayments;
    setTotalPayable(calculatedTotalPayable.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')); // Menambahkan koma setiap 3 digit
  };
  ...
  return (
    <div className="calculation-container">
      ...
      <div>
        ...
        <input type="text" id="interestRate" value={interestRate} onChange={handleInterestRateChange} />
      </div>
      <div>
        <label htmlFor="loanTenure">Loan Tenure (Years):</label>
        <input type="text" id="loanTenure" value={loanTenure} onChange={handleLoanTenureChange} />
      </div>
